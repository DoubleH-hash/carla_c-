...
class VisionDetectionArray {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.detections.forEach((val) => {
      bufferOffset = VisionDetection.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new VisionDetectionArray(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.detections[i] = VisionDetection.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.detections.forEach((val) => {
      length += VisionDetection.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new VisionDetectionArray(null);
    ...
    if (msg.detections !== undefined) {
      ...
      for (let i = 0; i < resolved.detections.length; ++i) {
        resolved.detections[i] = VisionDetection.Resolve(msg.detections[i]);
      }
    }
    else {
      resolved.detections = []
...
module.exports = VisionDetectionArray;
